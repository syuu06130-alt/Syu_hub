...
--
-- [追加機能 v2.1]
-- 14. Head Rock フローティングボタン（ドラッグ可能）
-- 15. 視点移動検知による Head Rock 一時停止（停止時間・再開遅延設定可能）
-- 16. リアルタイム円形ミニマップ（左下、サイズ・精度調整可能）
-- ============================================================
...
-- [追加] 視点移動検知関連の設定
local DirPauseSettings = {
    Enabled                = true,          -- 一時停止機能自体の有効/無効
    Sensitivity            = 2.0,           -- 移動検知の感度（度/フレーム）
    PauseDuration          = 0.3,           -- 移動検知後の一時停止時間（秒）
    ResumeDelay            = 0.2,           -- 移動停止後、再開までの遅延（秒）
}
...
-- [追加] 視点移動検知用変数
local lastCameraAngles = Vector2.new(Camera.CFrame.LookVector.X, Camera.CFrame.LookVector.Y) -- 近似値として使用
local cameraMoving     = false
local cameraStopTime   = 0
local dirLockPaused    = false
...
local MinimapDotContainer = nil
local MinimapDots = {}   -- プレイヤーごとのドットオブジェクト
...
-- [追加] 新設定タブ（ミニマップ・一時停止機能）
local ExtraTab      = Window:CreateTab("拡張機能",            4483345998)
...
local function LockToDirHead()
    ...
    -- 一時停止状態の処理
    if DirPauseSettings.Enabled then
        if dirLockPaused then
            if t < pauseEndTime then
                return  -- まだ一時停止中
            else
                dirLockPaused = false
            end
        elseif cameraMoving then
            -- 移動中はロック開始しない
            return
        end
    end
    ...
    dirLockConnection = RunService.RenderStepped:Connect(function()
        ...
        if DirPauseSettings.Enabled then
            if cameraMoving then
                dirLockPaused = true
                pauseEndTime = tick() + DirPauseSettings.PauseDuration
                ResetDirLock()
                return
            end
        end
...
local function UpdateCameraMovement()
    if not DirPauseSettings.Enabled then
        cameraMoving = false
        return
    end

    local currentLook = Camera.CFrame.LookVector
    local currentAngles = Vector2.new(currentLook.X, currentLook.Y)  -- 簡易比較
    local delta = (currentAngles - lastCameraAngles).Magnitude

    if delta > DirPauseSettings.Sensitivity * 0.01 then  -- 感度調整
        cameraMoving = true
        cameraStopTime = tick()
    elseif cameraMoving and (tick() - cameraStopTime) > DirPauseSettings.ResumeDelay then
        cameraMoving = false
    end
...
local function CreateFloatingButton()
    ...
    gui.Name = "SyuHub_FloatingButton"
    gui.Parent = CoreGui
    ...
    btn.MouseButton1Click:Connect(function()
        DirSettings.LockEnabled = not DirSettings.LockEnabled
        btn.Text = DirSettings.LockEnabled and "Head Rock ON" or "Head Rock OFF"
        btn.BackgroundColor3 = DirSettings.LockEnabled and Color3.fromRGB(50, 200, 50) or Color3.fromRGB(50, 120, 255)
        if not DirSettings.LockEnabled then ResetDirLock() end
    end)

    FloatingButtonGui = gui
    FloatingButton = btn
end
...
local function CreateMinimap()
    ...
    local gui = Instance.new("ScreenGui")
    gui.Name = "SyuHub_Minimap"
    gui.Parent = CoreGui
    gui.ResetOnSpawn = false

    local frame = Instance.new("Frame")
    frame.Size = UDim2.new(0, MinimapSettings.Size, 0, MinimapSettings.Size)
    frame.Position = UDim2.new(0.02, 0, 0.8, -MinimapSettings.Size/2)
    frame.BackgroundColor3 = Color3.new(0, 0, 0)
    frame.BackgroundTransparency = MinimapSettings.BackgroundTransparency
    ...
    dotContainer.Parent = canvas

    -- 自分を示す中心点
    local centerDot = Instance.new("Frame")
    centerDot.Size = UDim2.new(0, 6, 0, 6)
    centerDot.Position = UDim2.new(0.5, -3, 0.5, -3)
    centerDot.BackgroundColor3 = MinimapSettings.PlayerColor
    ...
    MinimapDotContainer = dotContainer
    ...
    for _, player in ipairs(Players:GetPlayers()) do
        if player ~= LocalPlayer then
            ...
            dot.BackgroundColor3 = MinimapSettings.EnemyColor
            dot.BorderSizePixel = 0
            dot.Visible = false
            dot.Parent = dotContainer
            Instance.new("UICorner", dot).CornerRadius = UDim.new(1, 0)
            MinimapDots[player] = dot
        end
    end
end

local function UpdateMinimap()
    if not MinimapSettings.Enabled or not MinimapGui then return end
    ...
    local scale = MinimapSettings.Scale
    local halfSize = MinimapSettings.Size / 2

    -- 表示対象プレイヤーを決定（ESPターゲット選択に従う）
    local showAll = (#Settings.ESPPlayers == 0)
    local targetSet = {}
    if not showAll then
        for _, name in ipairs(Settings.ESPPlayers) do
            targetSet[name] = true
    ...
    for player, dot in pairs(MinimapDots) do
        local shouldShow = showAll or targetSet[player.Name]
        if shouldShow and player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
            ...
            local relPos = localCF:PointToObjectSpace(enemyRoot.Position)
            -- relPos: X=右, Y=上, Z=前後。マップは上から見下ろすので X, Z を使用
            local mapX = relPos.X / scale
            local mapZ = -relPos.Z / scale  -- 前方が北なので、Zの負が上

            -- 円形クリッピング（半径以内なら表示）
            local distFromCenter = math.sqrt(mapX*mapX + mapZ*mapZ)
            if distFromCenter <= halfSize then
                dot.Position = UDim2.new(0.5, mapX - 3, 0.5, mapZ - 3)
                dot.Visible = true
            else
                dot.Visible = false
            end
    ...
    -- マップサイズ更新
    MinimapFrame.Size = UDim2.new(0, MinimapSettings.Size, 0, MinimapSettings.Size)
end
...
MainTab:CreateToggle({
    ...
    Callback = function(v)
        ...
        if FloatingButton then
            FloatingButton.Text = v and "Head Rock ON" or "Head Rock OFF"
            FloatingButton.BackgroundColor3 = v and Color3.fromRGB(50, 200, 50) or Color3.fromRGB(50, 120, 255)
        end
...
-- UI - 拡張機能タブ（新規）
-- ============================================================
ExtraTab:CreateSection("🧭 Head Rock 一時停止機能")
ExtraTab:CreateToggle({
    Name = "一時停止機能を有効にする",
    CurrentValue = true,
    Flag = "DirPauseEnabled",
    Callback = function(v) DirPauseSettings.Enabled = v end,
})
ExtraTab:CreateSlider({
    Name = "移動検知感度",
    Range = {0.5, 5}, Increment = 0.1, CurrentValue = 2.0,
    Flag = "DirPauseSensitivity",
    Callback = function(v) DirPauseSettings.Sensitivity = v end,
})
ExtraTab:CreateSlider({
    Name = "一時停止時間（秒）",
    Range = {0.1, 2}, Increment = 0.05, CurrentValue = 0.3,
    Flag = "DirPauseDuration",
    Callback = function(v) DirPauseSettings.PauseDuration = v end,
})
ExtraTab:CreateSlider({
    Name = "再開遅延（秒）",
    Range = {0, 1}, Increment = 0.05, CurrentValue = 0.2,
    Flag = "DirPauseResumeDelay",
    Callback = function(v) DirPauseSettings.ResumeDelay = v end,
})

ExtraTab:CreateSection("🗺️ ミニマップ設定")
ExtraTab:CreateToggle({
    Name = "ミニマップを表示",
    CurrentValue = true,
    Flag = "MinimapEnabled",
    Callback = function(v)
        MinimapSettings.Enabled = v
        if MinimapGui then MinimapGui.Enabled = v end
    end,
})
ExtraTab:CreateSlider({
    Name = "マップサイズ",
    Range = {80, 300}, Increment = 5, CurrentValue = 150,
    Flag = "MinimapSize",
    Callback = function(v) MinimapSettings.Size = v end,
})
ExtraTab:CreateSlider({
    Name = "縮尺（1スタッド = ?ピクセル）",
    Range = {10, 200}, Increment = 5, CurrentValue = 50,
    Flag = "MinimapScale",
    Callback = function(v) MinimapSettings.Scale = v end,
})
ExtraTab:CreateSlider({
    Name = "背景透明度",
    Range = {0, 1}, Increment = 0.05, CurrentValue = 0.8,
    Flag = "MinimapTransparency",
    Callback = function(v)
        MinimapSettings.BackgroundTransparency = v
        if MinimapFrame then MinimapFrame.BackgroundTransparency = v end
    end,
})
ExtraTab:CreateColorPicker({
    Name = "自分ドット色",
    Color = MinimapSettings.PlayerColor,
    Flag = "MinimapSelfColor",
    Callback = function(v) MinimapSettings.PlayerColor = v end,
})
ExtraTab:CreateColorPicker({
    Name = "敵ドット色",
    Color = MinimapSettings.EnemyColor,
    Flag = "MinimapEnemyColor",
    Callback = function(v) MinimapSettings.EnemyColor = v end,
})
...
InfoTab:CreateParagraph({
    Title   = "拡張機能",
    Content = "・フローティングボタン: 画面上のボタンでHead Rockを即ON/OFF、ドラッグ可能\n・一時停止機能: 視点を動かすと自動でHead Rockを一時停止\n・ミニマップ: 左下にリアルタイムで敵位置を表示"
})
...
RunService.RenderStepped:Connect(function()
    UpdateCameraMovement()   -- 視点移動検知
    LockToHead()
    LockToBot()
    LockToDirHead()
    UpdateMinimap()          -- ミニマップ更新
end)
...
local function Cleanup()
    ...
    if lockIndicator    then pcall(function() lockIndicator:Destroy()    end) end
    if dirLockIndicator then pcall(function() dirLockIndicator:Destroy() end) end
    if FloatingButtonGui then pcall(function() FloatingButtonGui:Destroy() end) end
    if MinimapGui       then pcall(function() MinimapGui:Destroy()       end) end
end
